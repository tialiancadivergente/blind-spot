...
const benefits = [
	...
	<>
		Apostila com 8 Ferramentas
		<br />
		de Desbloqueio Financeiro;
	</>,
	<>
		<strong className="font-bold">
			Garantia &apos;De Repente&apos; 120 dias:
		</strong>{" "}
		se você não receber nenhum dinheiro inesperado na sua conta bancária
		durante 120 dias após o Ponto Cego, nós vamos devolver o seu investimento
		no ingresso do evento;
	</>,
...
export default function SeventhSection() {
	...
	useEffect(() => {
		const interval = setInterval(
			() =>
				setActiveSlide(
					(current) => (current + 1) % testimonialPages.length,
				),
			4500,
		);
	...
	return (
		<section className="relative flex min-h-[1600px] w-full justify-center overflow-hidden bg-[url('/images/v2/setima_dobra_mobile.webp')] bg-[length:100%_1494px] bg-top bg-no-repeat md:min-h-[1751px] md:bg-[url('/images/v2/setima_dobra_desktop.webp')] md:bg-cover md:bg-center">
			<div className="relative min-h-[1600px] w-full max-w-[1200px] px-6 md:min-h-[1751px] md:px-0">
				...
				<div className="absolute left-1/2 top-[415px] flex -translate-x-1/2 flex-col items-center md:top-[700px]">
					<h3 className="text-center font-raleway text-[24px] font-extrabold leading-[36px] text-[#EBBA3D] md:text-[32px]">
						Executivo
					</h3>

					<p className="-mt-2 text-center font-raleway text-[14px] font-extrabold leading-[36px] text-white">
						(Ingresso Online)
					</p>
				</div>
				...
				<div className="absolute left-0 right-0 top-[535px] w-full px-5 md:top-[825px] md:px-0">
					<div className="flex w-full flex-col items-center md:flex-row md:justify-center md:gap-[68px]">
						<ul className="order-1 flex w-full max-w-[320px] flex-col gap-[12px] font-raleway text-[16px] font-medium leading-[20px] text-[#000C14] md:order-3 md:max-w-[378px] md:gap-[8px]">
							{benefits.map((benefit, index) => (
								...
								>
									<span
										aria-hidden="true"
										className="mt-[1px] block h-[18px] w-[18px] shrink-0 font-raleway text-[27px] font-medium leading-[18px] text-[#DB992F]"
									>
										✓
									</span>
						...
						<div className="order-2 mt-[58px] flex w-full max-w-[340px] flex-col items-center text-center font-raleway text-[#101B22] md:order-1 md:mt-0 md:w-[430px] md:max-w-none">
							...
							<div className="mt-[13px] flex items-end justify-center">
								<span className="mr-[7px] pb-[4px] text-[16px] font-bold leading-[24px]">
									R$
								</span>

								<strong className="text-[72px] font-bold leading-[51px] tracking-[-1px]">
									47,00
								</strong>
							</div>

							<p className="mt-[22px] w-full whitespace-nowrap text-[14px] font-medium leading-[24px] md:text-[16px]">
								(Cupom de 90% de desconto já aplicado)
							</p>

							<Link
								href="https://pay.hotmart.com/T101582944K?checkoutMode=10"
								className="mt-[18px] flex h-[54px] w-[294px] max-w-full items-center justify-center rounded-[50px] border-x border-y-0 border-solid border-[#25D366] px-[10px] font-raleway text-[16px] font-bold leading-[24px] text-black shadow-[0_-0.14px_1.92px_#25D36626,0_-0.35px_4.86px_#25D36636,0_-0.71px_9.92px_#25D36644,0_-1.46px_20.44px_#25D36654,0_-4px_56px_#25D3667A] md:mt-[32px] md:w-[319px] md:uppercase"
								style={{
									background:
					...
					<div className="mt-[60px] w-full md:mt-[108px]">
						<h3 className="mx-auto w-[241px] text-center font-raleway text-[18px] font-extrabold leading-[24px] text-[#00121F] md:w-[723px] md:text-[24px] md:leading-[30px]">
							Depoimentos de quem{" "}
							<span className="block md:inline">assistiu online:</span>
						</h3>

						<div className="mx-auto mt-[36px] w-full max-w-[420px] overflow-hidden md:mt-[72px] md:max-w-[1200px] md:pl-[80px]">
							<div
								key={activeSlide}
								className="flex w-max items-start gap-[28px] transition-opacity duration-500 md:gap-[68px]"
							>
								{testimonialPages[activeSlide].map((image, index) => (
									<img
										key={image}
										src={`/images/v2/Group ${image}.svg`}
										alt={`Depoimento de quem assistiu online ${
											activeSlide * 5 + index + 1
										}`}
										draggable={false}
										className="h-auto w-[280px] shrink-0 select-none object-contain object-top md:h-[220px] md:w-[220px]"
									/>
								))}
							</div>
						</div>
